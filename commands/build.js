        ...
        var dockerImagePath =
          taskProps["docker.image.path"] !== undefined
            ...
            : taskProps["team.name"]
              ...
              .replace(/[^a-zA-Z0-9\-]/g, "")
              .toLowerCase();
        await exec(`${shellDir}/build/package-docker.sh "${dockerImageName}" "${taskProps["version.name"]}" "${dockerImagePath}" "${JSON.stringify(taskProps["global/container.registry.host"])}" "${taskProps["global/container.registry.port"]}" "${taskProps["global/container.registry.user"]}" "${taskProps["global/container.registry.password"]}" "${JSON.stringify(taskProps["global/artifactory.url"])}" "${taskProps["global/artifactory.user"]}" "${taskProps["global/artifactory.password"]}" "${JSON.stringify(taskProps["build.container.registry.host"])}" "${taskProps["build.container.registry.port"]}" "${taskProps["build.container.registry.user"]}" "${taskProps["build.container.registry.password"]}" "${dockerFile}"`);
      }
    } catch (e) {
